...
if __name__ == '__main__':
    ...
    departmentList = []
    with open('data/school_department.csv') as f:
        f_csv = csv.reader(f)
        for row in f_csv:
            if row != []:
                if row[0] == schoolname:
    ...
    departmentNumUpperBound = len(departmentList) - 2

    workbook = xlwt.Workbook(encoding = 'utf-8')

    for grade in range(2000, 2020):
        worksheet = workbook.add_sheet(str(grade) + r'届')

        worksheet.write(0, 0, r'姓名')
        worksheet.write(0, 1, r'学号')
        worksheet.write(0, 2, r'院系')
